...
import { ZonesService } from '../growth/zones.service';
...
export class SurgeService {
  ...
  async getMultiplier(city: string, lat: number, lon: number): Promise<number> {
    ...
    const zones = await this.zones.resolvePoint(lat, lon).catch(() => []);
    const zoneFloor = Math.max(
      1,
      ...zones
        .filter((zone) => zone.areaType === 'SURGE_ZONE')
        .map((zone) => Number(zone.minSurgeMultiplier ?? 1)),
    );
    let multiplier = zoneFloor;
    if (ratio > this.demandThreshold) {
      ...
      multiplier = Math.min(
        this.maxMultiplier,
        Math.max(zoneFloor, 1 + steps * this.multiplierStep),
      );
    }
